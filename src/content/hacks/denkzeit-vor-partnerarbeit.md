---
title: Denkzeit vor Partner- oder Gruppenarbeit
summary: Kurze Phase des alleinigen Arbeitens vor dem Austausch erhöht die Chance auf eigene Strategien — und damit auf spätere Vielfalt im Plenum.
tags:
  - Plenum
  - Aktivierung
  - Sozialform
order: 27
quiz:
  - question: "Warum ist unabhängiges Arbeiten vor der Partnerarbeit für eine spätere Diskussion hilfreich?"
    options:
      - "Weil jede Person erst eine eigene Spur entwickeln kann — sonst übernimmt oft schnell eine vorherrschende Idee."
      - "Weil Partnerarbeit ohnehin immer zu lange dauert."
      - "Weil allein arbeiten keine mathematischen Fehler erzeugt."
    correctIndex: 0
    explanation: "Eigenes Denken zuerst steigert Vielfalt und Verständlichkeit der späteren Präsentationen."
  - question: "Was ist eine pragmatische Leitplanke für die erste Denkphase?"
    options:
      - "Mindestens 45 Minuten, bis alle fertig sind."
      - "Knappe, klar begrenzte Zeit mit einer konkreten Mini-Aufgabe („notiere einen Weg / eine Skizze“)."
      - "Gar keine Zeit — sofort in Gruppen, damit es lebendig wird."
    correctIndex: 1
    explanation: "Kurz und zielgebunden verhindert Leerlauf und hält die kognitive Anforderung."
  - question: "Fallbeispiel: In jeder Gruppe steht nach 30 Sekunden dieselbe Lösung. Was ist eine plausible Ursache?"
    options:
      - "Die Aufgabe war zu schwer."
      - "Es gab keine Phase, in der jede Person für sich denken musste — eine dominante Idee hat sofort gewonnen."
      - "Zu wenig Plakatpapier."
    correctIndex: 1
    explanation: "Ohne Denkzeit für alle klont sich oft der schnellste Vorschlag."

---

## Kernaussage

**Erst denken, dann tauschen** — sonst tauscht die Gruppe oft nur **eine** fertige Idee aus.

## Was ist das?

Ein pragmatischer Baustein aus den Erfahrungen der Five-Practices-Arbeit: Lernende brauchen **Zeit für unabhängiges Denken**, bevor sie in Paaren oder Gruppen diskutieren. So entstehen **unterschiedliche Ansätze**, die du beobachten, auswählen und im Plenum **sequenziell verknüpfen** kannst. Ohne diese Phase neigen Gruppen dazu, **sofort** den ersten Vorschlag zu übernehmen.

## Warum ist das gut?

Mehr **echte** Vielfalt in der Erarbeitung → mehr Material für **Selecting** und **Connecting** → mathematisch dichteres Plenum.

## Wie geht das im Unterricht?

- **So gehst du vor:** Gib die Aufgabe, erkläre knapp das Ziel, dann **2–5 Minuten Stillarbeit** (je nach Aufgabe): „Jede Person: einen Lösungsweg oder eine Skizze — noch nicht abstimmen.“
- **Achte dabei auf:** Klare Anweisung, **was** aufgeschrieben werden soll (nicht nur „überlegt mal“).
- **Prüfe am Ende:** Siehst du bei Monitoring **mehr als eine** tragfähige Spur pro Gruppe?

## Beispiele aus dem Unterricht

1. **Funktion:** Erst allein: „Was fällt dir am Graphen auf?“ — dann Partner: eine gemeinsame Liste — dann Plenum.
2. **Geometrie:** Erst allein eine kleine Skizze und ein Stichwort zur Begründung — dann Tischabgleich.
3. **Textaufgabe:** Erst allein markieren, **was** gegeben und **was** gesucht ist — dann Gruppe: Gleichung oder Modell.

## Was kann eine Lehrkraft dabei falsch machen?

- **Zu kurz** abfedern („10 Sekunden, dann Partner“) — dann entsteht keine eigene Spur.
- **Zu lang** ohne Zwischenblick — dann verlieren Unsichere den Anschluss.

## Was kann in der Praxis schiefgehen?

- Kultur der **sofortigen Hilfe**: Du musst die **Regel** „erst allein“ mehrfach einüben und vorleben.

## Querverweise

- [Arbeitsphasen zielgerichtet beobachten](../arbeitsphasen-zielgerichtet-beobachten/)
- [Beiträge fürs Plenum auswählen](../beitraege-fuers-plenum-auswaehlen/)
- [Kognitive Last senken](../kognitive-last/)

## Quelle (Hintergrund)

Smith, M. S., & Stein, M. K. (2018). *5 Practices …* (2nd ed., Kap. 9 „Lesson 3“). **Paraphrasiert**; Auszug aus privatem HTML-Export.
